...
pthread_cond_t maxChilds;
pthread_cond_t noChilds;
...
int main(int argc, char **argv){
	...
	while (1) {
		...
		if (strcmp(arg_vector[0], EXIT_COMMAND) == 0 ) {
			...
			pthread_cond_destroy_(&maxChilds);
			pthread_cond_destroy_(&noChilds);
			fclose(log_fd);
			lst_destroy(lst);
			exit(EXIT_SUCCESS);
		}
		...
		* else wait here */
		pthread_cond_wait_(&maxChilds, &mutex);
...
void *monitor(void) {
	...
	while (1) {
		/* wait for a new child process */
		pthread_cond_wait_(&noChilds, &mutex);
		...
		if (child_count > 0) {
			...
			pthread_cond_signal_(&maxChilds);
		}
